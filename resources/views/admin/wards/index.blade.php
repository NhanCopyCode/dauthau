    ...
    <div class="box">
        <div class="content-header border-bottom pb-5">
            ...
            @can('WardController@store')
                <a href="{{ url('/admin/wards/create') }}" class="btn btn-default float-right" title="{{ __('message.new_add') }}">
                            ...
                            <td class="dropdown text-center">
                                ...
                                <div class="dropdown-menu p-0">
                                    @can('WardController@show')
                                        <a href="{{ url('/admin/wards/' . $item->id) }}" title="{{ __('Xem') }}"><button
                                                class="btn btn-info dropdown-item"><i class="fas fa-eye"></i>
                                                {{ __('message.user.view_user') }}</button></a>
                                    @endcan
                                    @can('WardController@update')
                                        <a href="{{ url('/admin/wards/' . $item->id . '/edit') }}"
                                            title="{{ __('Sửa') }}"><button class="btn btn-primary dropdown-item"><i
                                                    class="far fa-edit" aria-hidden="true"></i>
                                                {{ __('message.user.edit_user') }}</button></a>
                                    @endcan
                                    @can('WardController@destroy')
                                        ...
                                        {!! Form::button('<i class="far fa-trash-alt" aria-hidden="true"></i> ' . __('Xóa'), [
                                            'type' => 'submit',
